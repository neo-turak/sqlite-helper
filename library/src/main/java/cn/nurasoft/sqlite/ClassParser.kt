...
import cn.nurasoft.sqlite.JavaSqliteUtils.PRIMITIVES_TO_WRAPPERS
...
internal fun <T> classParser(clazz: Class<T>): RowParser<T> {
    ...
    if (applicableConstructors.isEmpty()) {
        throw BasicException("Can't initialize object parser for ${clazz.canonicalName}, no acceptable constructors found")
    }
    ...
    val preferredConstructor = if (applicableConstructors.size > 1) {
        applicableConstructors
                ...
                .singleOrNull()
                ?: throw BasicException("Several constructors are annotated with ClassParserConstructor")
    } else {
    ...
    return object : RowParser<T> {
        ...
        override fun parseRow(columns: Array<Any?>): T {
            if (parameterTypes.size != columns.size) {
                ...
                throw BasicException("Class parser for ${preferredConstructor.name} " +
...
private fun castValue(value: Any?, type: Class<*>): Any? {
    if (value == null && type.isPrimitive) {
        throw BasicException("null can't be converted to the value of primitive type ${type.canonicalName}")
    }
    ...
    if (value is Long) {
        if (type == Integer.TYPE || type == Integer::class.java) {
            return value.toInt()
            ...
            return value != 0L
        } else if (type == Character.TYPE || type == Character::class.java) {
            return value.toChar()
        }
    }
    ...
    if (value is Int) {
        if (type == java.lang.Long.TYPE || type == java.lang.Long::class.java) {
            ...
            return value != 0
        } else if (type == Character.TYPE || type == java.lang.Character::class.java) {
            return value.toChar()
        }
    }

    if (value is String && value.length == 1
            && (type == Character.TYPE || type == java.lang.Character::class.java)) {
        return value[0]
    }

    throw BasicException("Value $value of type ${value::class.java} can't be cast to ${type.canonicalName}")
}
