...
fun <T: Any> Cursor.parseSingle(parser: RowParser<T>): T = CursorHelper.useCursor(this) {
    if (count != 1)
...
fun <T: Any> Cursor.parseOpt(parser: RowParser<T>): T? = CursorHelper.useCursor(this) {
    if (count > 1)
...
fun <T: Any> Cursor.parseList(parser: RowParser<T>): List<T> = CursorHelper.useCursor(this) {
...
fun <T: Any> Cursor.parseSingle(parser: MapRowParser<T>): T = CursorHelper.useCursor(this) {
    if (count != 1)
...
fun <T: Any> Cursor.parseOpt(parser: MapRowParser<T>): T? = CursorHelper.useCursor(this) {
    if (count > 1)
...
fun <T: Any> Cursor.parseList(parser: MapRowParser<T>): List<T> = CursorHelper.useCursor(this) {
    val list = ArrayList<T>(count)
    moveToFirst()
    while (!isAfterLast) {
