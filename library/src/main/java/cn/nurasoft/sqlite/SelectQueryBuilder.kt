...
abstract class SelectQueryBuilder(private val tableName: String) {
    ...
    fun <T> exec(f: Cursor.() -> T): T {
        val cursor = doExec()
        return CursorHelper.useCursor(cursor) {
            cursor.f()
        }
    }

    inline fun <T: Any> parseSingle(parser: RowParser<T>): T = CursorHelper.useCursor(doExec()) {
        it.parseSingle(parser)
    }

    inline fun <T: Any> parseOpt(parser: RowParser<T>): T? = CursorHelper.useCursor(doExec()) {
        it.parseOpt(parser)
    }

    inline fun <T: Any> parseList(parser: RowParser<T>): List<T> = CursorHelper.useCursor(doExec()) {
        it.parseList(parser)
    }

    inline fun <T: Any> parseSingle(parser: MapRowParser<T>): T = CursorHelper.useCursor(doExec()) {
        it.parseSingle(parser)
    }

    inline fun <T: Any> parseOpt(parser: MapRowParser<T>): T? = CursorHelper.useCursor(doExec()) {
        it.parseOpt(parser)
    }

    inline fun <T: Any> parseList(parser: MapRowParser<T>): List<T> = CursorHelper.useCursor(doExec()) {
        it.parseList(parser)
    }
    ...
    fun having(having: String): SelectQueryBuilder {
        if (havingApplied) {
            throw SQLException("Query having was already applied.")
        }
    ...
    fun whereArgs(select: String, vararg args: Pair<String, Any>): SelectQueryBuilder {
        if (selectionApplied) {
            throw SQLException("Query selection was already applied.")
        }
